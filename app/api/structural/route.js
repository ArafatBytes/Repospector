...
export async function GET(req) {
  try {
    ...
    // Fetch reports
    const reports = await StructuralReport.find(query).sort({ createdAt: -1 });

    return NextResponse.json(reports);
  } catch (error) {
    console.error("Error fetching structural reports:", error);
    return NextResponse.json(
...
export async function POST(req) {
  try {
    ...
    // Create new report
    const newReport = await StructuralReport.create({
      ...data,
      userId,
    });
    ...
    console.error("Error creating structural report:", error);
    return NextResponse.json(
