...
export async function POST(request) {
  try {
    ...
    // Create new facade report document
    const report = await FacadeReport.create({
      ...formData,
      userId,
    });
    ...
    return NextResponse.json({
      message: "Facade Report submitted successfully",
      report,
    });
  } catch (error) {
    console.error("Error submitting facade report:", error);
    return NextResponse.json(
      { error: "Error submitting facade report" },
      { status: 500 }
    );
  }
}
...
export async function GET(request) {
  try {
    ...
    const reports = await FacadeReport.find({
    ...
    console.error("Error fetching facade reports:", error);
    return NextResponse.json(
      { error: "Failed to fetch facade reports" },
      { status: 500 }
    );
  }
}
