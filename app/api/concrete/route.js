...
export async function POST(request) {
  try {
    ...
    // Create new concrete report document
    const report = await ConcreteReport.create({
      ...formData,
      userId,
    });
    ...
    return NextResponse.json({
      message: "Concrete Report submitted successfully",
      report,
    });
  } catch (error) {
    console.error("Error submitting concrete report:", error);
    return NextResponse.json(
      { error: "Error submitting concrete report" },
      { status: 500 }
    );
  }
}
...
export async function GET(request) {
  try {
    ...
    const reports = await ConcreteReport.find({
    ...
    console.error("Error fetching concrete reports:", error);
    return NextResponse.json(
      { error: "Failed to fetch concrete reports" },
      { status: 500 }
    );
  }
}
