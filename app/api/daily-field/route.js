...
export async function POST(request) {
  try {
    ...
    // Create new daily field report document
    const report = await DailyFieldReport.create({
      ...formData,
      userId,
    });
    ...
    console.error("Error submitting daily field report:", error);
    return NextResponse.json(
      { error: "Error submitting daily field report" },
      { status: 500 }
    );
  }
}
...
export async function GET(request) {
  try {
    ...
    const reports = await DailyFieldReport.find({
    ...
    console.error("Error fetching daily field reports:", error);
    return NextResponse.json(
      { error: "Failed to fetch daily field reports" },
      { status: 500 }
    );
  }
}
