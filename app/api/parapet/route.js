...
export async function GET(req) {
  try {
    ...
    // Fetch reports
    const reports = await ParapetReport.find(query).sort({ createdAt: -1 });

    return NextResponse.json(reports);
  } catch (error) {
    console.error("Error fetching parapet reports:", error);
    return NextResponse.json(
...
export async function POST(req) {
  try {
    ...
    // Create new report
    const newReport = await ParapetReport.create({
      ...data,
      userId,
    });
    ...
    console.error("Error creating parapet report:", error);
    return NextResponse.json(
